...
- AND the `Deploy OpenShift Environment` and `OpenShift` jobs in `e2e.yml`
  SHALL also run, using a `hypershell-ci-mq-<short-sha>` namespace as the CI
  E2E Workflow requirement defines
- AND the ephemeral pull-request environment workflow
  (`ephemeral-pr-environments.spec.md`) SHALL NOT run: a merge-queue entry
  has no pull-request number, no GitHub-brokered OAuth, and no access
  comment
